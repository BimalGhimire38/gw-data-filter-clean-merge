...
new_dtw_ws.title = "STW_Supplementry_Data"
...
for file in xlsx_files:
    ...
    for i in range(1,ws.max_row+1):
        well_identifier = ws.cell(row=i,column=2).value
        if "STW" in str(well_identifier):
            ...
            for k in range(6,18):
                ...
                if month_short_form:  # Check if month_short_form is not None
                    ...
                    # Handle the case where month_short_form is None, perhaps by skipping or setting a default value
                    formatted_date = None  # or some default value
                    gw_value = ws.cell(row=i, column=k).value
                    value_only_dict[formatted_date] = gw_value  # or handle it differently
...
new_dwb.save("DTW_NEW.xlsx")
new_swb.save("STW_NEW.xlsx")
